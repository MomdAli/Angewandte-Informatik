...
![[Assets/Kernel and User mode.png|center]]
...
- The [[Studium/Betriebssysteme/Other/Trap Instruction]] allows a user-mode program to request a privileged operation from the OS safely. When a program makes a **system call** (e.g., reading a file), it uses a [[Studium/Betriebssysteme/Other/Trap Instruction]] to switch from user mode to kernel mode.
...
- Without traps, the OS would need to simulate everything (like hardware access), which would be slow and inefficient. The [[Studium/Betriebssysteme/Other/Trap Instruction]] allows user-mode programs to run directly on the CPU at full speed while giving the OS control only when needed (like during system calls).
...
##### Next chapter: [[Studium/Betriebssysteme/OSTEP/4_Scheduling\|4.Scheduling]]