...
public class IndexMinPQ<Key, PrioValue> {
	...
	private void upheap(int i) {
		...
		while (i > 0 && cmp.compare(prioPos.get(heap[(i - 1) / 2]).prio,
				prioPos.get(key).prio) > 0) {
			heap[i] = heap[(i - 1) / 2];
			// pos[heap[i]] = i;
	...
	private void downheap(int i) {
		...
		while (2 * i + 1 < size) // heap[i] hat linkes Kind
		{
			...
			if (j + 1 < size) { // heap[i] hat rechtes Kind
				...
				if (cmp.compare(prioPos.get(heap[j + 1]).prio,
						prioPos.get(heap[j]).prio) < 0)
					j++;
			}
			// heap[j] ist jetzt das kleinste Kind
			// if (data[key] <= data[heap[j]]) {
			if (cmp.compare(prioPos.get(key).prio,
					prioPos.get(heap[j]).prio) <= 0) {
				break;
			}
	...
	/**
	 * Prüft, ob der Schlüssel in der Prioritätsliste enthalten ist.
	 * @param k Schlüssel.
	 * @return true, falls der Schlüssel enthalten ist, sonst false.
	 */
	public boolean contains(Key k) {
		return prioPos.containsKey(k);
	}
	...
	public String toString() {
		...
		for (int i = 0; i < size; i++)
			sb.append("(").append(heap[i]).append(",")
					.append(prioPos.get(heap[i]).prio).append("),");
