...
public class ScotlandYard {
	...
	static {
		...
		try {
			sySpAStar = new ShortestPath<>(syGraph,
					new ScotlandYardHeuristic());
		} catch (FileNotFoundException e) {
			throw new RuntimeException(e);
		}
	}
	private static ShortestPath<Integer> sySpDijkstra = new ShortestPath<>(
			syGraph, null); // Dijkstra
	...
	private static void shortestPath(Integer u, Integer v, Double dist) {
		...
		System.out
				.println("Distance (Dijkstra) = " + sySpDijkstra.getDistance());
	...
	private static void shortestPathWithSimulation(Integer u, Integer v,
			Double dist) {
	 ...
	 * @throws FileNotFoundException
	 */
	public static UndirectedGraph<Integer> getGraph()
			throws FileNotFoundException {
		...
		System.out.println(
				"Number of Vertices: " + sy_graph.getNumberOfVertexes()); // 199
		System.out
				.println("Number of Edges:    " + sy_graph.getNumberOfEdges()); // 431
		double wSum = 0.0;
